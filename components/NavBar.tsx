...
const NavBar = ({cart}:IPropType) => {  
  return <nav className='container flex top-0 right-0 left-0 bg-white  mt-0 items-center justify-between py-1 px-5  mx-auto md:flex-row lg:flex-row flex-col z-[990] mb-3'>
        <Link href="/" >
            <div className='flex items-center cursor-pointer' >
                <Image src="/logo.png" alt="logo" width="35" height="35" />
                <span className='font-bold text-3xl italic ml-2 text-primary'>BMS Store</span>
            </div>
        </Link>
        ...
        <li className='mr-6 font-medium text-gray-600'>
                <span className='flex'>
                <Link href="/viewcart"><Image src="/top-header.png" alt="" width={55} height={55} className='object-cover md:w-10 md:h-10 lg:w-20 lg:h-20'/></Link>
                <span className='bg-primary rounded-full text-white w-5 h-5 lg:w-7 lg:h-7 text-center'>{cart.length}</span>
                </span>
